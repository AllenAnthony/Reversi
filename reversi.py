...
def game(white_engine, black_engine, game_time=60.0, verbose=False):
    ...
    for move_num in range(60):
        moves = []
        for color in [-1, 1]:
            start_time = timeit.default_timer() #返回当前时间 timeit.default_timer()单位是秒
...
if __name__=='__main__':
    engines_b = __import__('engines.XZ')
    engines_w = __import__('engines.MCTS')
    engine_b = engines_b.__dict__['XZ'].__dict__['engine']()
    engine_w = engines_w.__dict__['MCTS'].__dict__['engine']()
    main(engine_w, engine_b, game_time=60, verbose=True)
