...
class Board(object):
    ...
    def starting_state(self):
        # p1 placed(black), p2 placed(white), previous player, player to move # 2 correspond white and 1 correspond black
    ...
    def pack_state(self, data): #将坐标转换为二进制
        player = data['player']
    ...
    def unpack_state(self, state): #将二进制转换为坐标
        p1_placed, p2_placed, previous, player = state
    ...
    def pack_action(self, notation): #记号转换为坐标
    ...
    def unpack_action(self, action): #坐标转换为记号
        if action is None:
            return ''
        r, c = action
