...
class UCT(object): #父类
    def __init__(self, board, **kwargs):
        self.board = board
        self.history = [] #存储states 一个state:(state[1], state[2], previous, player)
        self.stats = {} #每调用一次get_action()就清空一次记录 当次get_action()所探索到的所有(player, state)#当前player和他导致的下一个状态
    ...
    def display(self, state, action):
        state = self.board.pack_state(state) #将坐标转换为二进制
        action = self.board.pack_action(action) #记号转换为坐标
        return self.board.display(state, action)

    def winner_message(self, winners): #打印胜者信息
    ...
    def get_action(self):
        ...
        self.max_depth = 0
        self.data = {}
        self.stats.clear() #每调用一次get_action()，就清空一次记录

        state = self.history[-1]
        player = self.board.current_player(state)
        legal = self.board.legal_actions(self.history[:]) #返回坐标的集合
        ...
        return self.board.unpack_action(self.data['actions'][0]['action']) #将二进制转换为坐标
    ...
    def run_simulation(self):
        ...
        for t in xrange(1, self.max_actions + 1):
            ...
            if all((player, S) in stats for p, S in actions_states):
                ...
                log_total = log(sum(stats[(player, S)].visits for p, S in actions_states) or 1)
...
class UCTWins(UCT):#按最终结果评估
    action_template = "{action}: {percent:.2f}% ({wins} / {plays})"

    def __init__(self, board, **kwargs):
        super(UCTWins, self).__init__(board, **kwargs)
        self.end_values = board.win_values #最终结果

    def calculate_action_values(self, state, player, legal): #在当前状态下对每个动作的评估
